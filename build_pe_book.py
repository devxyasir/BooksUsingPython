#!/usr/bin/env python3
"""
Prompt Engineering Complete Guide — Beautiful PDF Book Generator
"""
...
P = {
    'cover':        HexColor('#0D0A1F'),
    'cover_mid':    HexColor('#1A1040'),
    'cover_accent': HexColor('#7C3AED'),
    'primary':      HexColor('#7C3AED'),
    'primary_d':    HexColor('#5B21B6'),
    'primary_l':    HexColor('#F5F3FF'),
    'dark':         HexColor('#0F0A1E'),
    'heading':      HexColor('#1E1B4B'),
    'sub':          HexColor('#3730A3'),
    'text':         HexColor('#374151'),
    'muted':        HexColor('#9CA3AF'),
    'code_bg':      HexColor('#F8F7FF'),
    'code_bd':      HexColor('#C4B5FD'),
    'code_txt':     HexColor('#1E1B4B'),
    ...
    'tbl_hdr':      HexColor('#7C3AED'),
    'tbl_even':     HexColor('#FAFAFA'),
    'tbl_odd':      HexColor('#FFFFFF'),
    'border':       HexColor('#E5E7EB'),
    'rule':         HexColor('#DDD6FE'),
    'sec_accent':   HexColor('#A78BFA'),
}
...
def cover_canvas(cv, doc):
    ...
    # Left accent
    cv.setFillColor(HexColor('#2E1065'))
    cv.rect(0, 0, 4*mm, PAGE_H, fill=1, stroke=0)
    # Dot grid
    cv.setFillColor(HexColor('#1E1050'))
    for row in range(7):
        for col in range(5):
            cv.circle(ML + col*16*mm, PAGE_H - 70*mm - row*16*mm, 2, fill=1, stroke=0)
    # Watermark
    cv.setFillColor(HexColor('#130D30'))
    cv.setFont('Helvetica-Bold', 130)
    cv.drawString(8*mm, PAGE_H/2 - 50*mm, 'PROMPT')
    # Eyebrow
    cv.setFillColor(HexColor('#A78BFA'))
    ...
    cv.drawString(ML, PAGE_H - 108*mm, 'Prompt')
    cv.setFillColor(HexColor('#C4B5FD'))
    cv.setFont('Helvetica-Bold', 40)
    cv.drawString(ML, PAGE_H - 128*mm, 'Engineering')
    # Rule
    cv.setFillColor(P['cover_accent'])
    ...
    cv.drawString(ML, PAGE_H - 150*mm, 'From Beginner to Production-Grade AI System Designer')
    # Badges
    bx = ML
    by = PAGE_H - 175*mm
    for badge in ['13 Sections', 'Design Patterns', 'Advanced Techniques', 'Security', 'Evaluation']:
        bw = len(badge) * 5.8 + 14
        cv.setFillColor(HexColor('#2E1065'))
        cv.roundRect(bx, by - 4, bw, 17, 4, fill=1, stroke=0)
        cv.setStrokeColor(HexColor('#7C3AED'))
        cv.setLineWidth(0.6)
    ...
    cv.drawString(ML, PAGE_H - 195*mm, 'Mastering the art and science of communicating with Large Language Models.')
    cv.drawString(ML, PAGE_H - 205*mm, 'From prompt basics to autonomous agent design.')
    # Bottom bar
    cv.setFillColor(HexColor('#080612'))
    cv.rect(0, 0, PAGE_W, 42*mm, fill=1, stroke=0)
    cv.setStrokeColor(HexColor('#2E1065'))
    cv.setLineWidth(0.5)
    cv.line(0, 42*mm, PAGE_W, 42*mm)
    cv.setFillColor(HexColor('#475569'))
    cv.setFont('Helvetica', 8)
    topics = 'CoT  |  ReAct  |  Tree-of-Thought  |  Meta Prompting  |  Agentic Systems  |  Security  |  Evaluation  |  Blueprints'
...
def content_canvas(cv, doc):
    ...
    cv.drawString(ML, PAGE_H - 13*mm, 'The Complete Guide to Prompt Engineering')
...
def fmt(text):
    ...
    text = re.sub(r'\*([^*\n]+?)\*', r'<i>\1</i>', text)
    text = re.sub(r'`([^`]+)`', lambda m: '<font name="Courier" size="9" color="#1E1B4B">' + m.group(1) + '</font>', text)
    return text
...
TOC_DATA = [
    ("1", "Foundations", ["What is a Prompt?", "What is Prompt Engineering?", "Why Prompts Matter", "How LLMs Interpret Prompts", "Prompt vs Code vs Fine-tuning", "The Prompt Formula Blueprint"]),
    ("2", "How LLMs Understand Prompts", ["Tokenization", "Attention & Word Order", "Why Wording Changes Output", "Role Conditioning", "Instruction Hierarchy"]),
    ("3", "Core Prompt Components", ["Instruction Clarity", "Context Injection", "Constraints: Hard vs Soft", "Output Formatting", "Role Prompting", "Few-Shot Prompting"]),
    ("4", "Types of Prompts", ["Instructional", "Conversational", "Role-Based", "Chain-of-Thought", "Multi-Step", "Structured Output", "Agentic"]),
    ("5", "Advanced Techniques", ["CoT Deep Dive", "Self-Consistency", "Tree-of-Thought", "ReAct Pattern", "Prompt Decomposition", "Meta Prompting", "Self-Critique"]),
    ("6", "Prompt Design Patterns", ["Input→Transform→Output", "Multi-Step Pipeline", "Planner→Executor", "Router Pattern", "Memory-Augmented", "Tool-Using Prompts"]),
    ("7", "Optimization & Debugging", ["Why Prompts Fail", "Iterative Improvement Loop", "Diagnosing Failures", "A/B Testing", "Quality Checklist"]),
    ("8", "Types of Prompt Systems", ["Naive System", "Structured System", "Multi-Agent System", "Tool-Augmented System", "Autonomous Agent"]),
    ("9", "Real-World Applications", ["AI Chatbot", "Customer Support", "Code Assistant", "Research Assistant", "Content Pipeline", "Resume Screening"]),
    ("10", "Optimization at Scale", ["Latency vs Complexity", "Token Cost", "Caching Strategies", "Modular Design", "Versioning", "Evaluation at Scale"]),
    ("11", "Prompt Evaluation", ["Five Core Metrics", "Accuracy Measurement", "Hallucination Detection", "Consistency", "Full Evaluation Pipeline"]),
    ("12", "Security & Risks", ["Prompt Injection", "Jailbreaks", "Data Leakage", "Safe Design Principles", "Guardrails"]),
    ("13", "Practical Blueprints", ["ChatGPT-Style Assistant", "AI Tutor System", "Document Q&A", "Coding Assistant", "Autonomous Agent"]),
]
...
def build_book():
    OUT = 'Books/PromptEngineering_Complete_Guide_Book.pdf'
    S = make_styles()
    ...
    for path in ['PromptEngineering/pe_guide_part1.md', 'PromptEngineering/pe_guide_part2.md']:
